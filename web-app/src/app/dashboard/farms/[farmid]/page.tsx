  ...
  useEffect(() => {
    ...
    fetch("/api/get-farm", {
      ...
      .then((data) => {
        ...
        setTotalPlants(Object.keys(data.farm.plants).length);
      })
  ...
  useEffect(() => {
    ...
    const getPastStats = async () => {
      try {
        ...
        // Loop through plants to check climate alerts for each one
        Object.keys(farm.plants).forEach((plant) => {
          const cropAttr = (cropAttributes as any)[plant]; // Assume cropAttributes is a predefined object
          const {
            optimalTempRangeC,
            optimalPrecipitation,
            optimalSoilMoisture,
          } = cropAttr;

          // Temperature alert
          if (
            data.averageTemperatureC < optimalTempRangeC[0] ||
            data.averageTemperatureC > optimalTempRangeC[1]
          ) {
            setAlerts((prevAlerts) => [
              ...prevAlerts,
              {
                event: "Temperature Alert",
                headline: `${plant} temperature is outside optimal range`,
                desc: `The average temperature for ${plant} is ${data.averageTemperatureC}°C, which is outside the optimal range of ${optimalTempRangeC[0]}°C to ${optimalTempRangeC[1]}°C.`,
              },
            ]);
          }
          ...
          // Precipitation alert
          if (
            data.averagePrecipitationMm < optimalPrecipitation[0] ||
            data.averagePrecipitationMm > optimalPrecipitation[1]
          ) {
            setAlerts((prevAlerts) => [
              ...prevAlerts,
              {
                event: "Precipitation Alert",
                headline: `${plant} precipitation is outside optimal range`,
                desc: `The average precipitation for ${plant} is ${data.averagePrecipitationMm}mm, which is outside the optimal range of ${optimalPrecipitation[0]}mm to ${optimalPrecipitation[1]}mm.`,
              },
            ]);
          }
          ...
          // Soil moisture alert
          if (
            data.averageSoilMoisture < optimalSoilMoisture[0] ||
            data.averageSoilMoisture > optimalSoilMoisture[1]
          ) {
            setAlerts((prevAlerts) => [
              ...prevAlerts,
              {
                event: "Soil Moisture Alert",
                headline: `${plant} soil moisture is outside optimal range`,
                desc: `The average soil moisture for ${plant} is ${data.averageSoilMoisture}, which is outside the optimal range of ${optimalSoilMoisture[0]} to ${optimalSoilMoisture[1]}.`,
              },
            ]);
          }
        });
        ...
        <section className={styles.super_section}>
          ...
          <section className={styles.section}>
            <h2 className={styles.sectionTitle}>
              {farm.address || "Address not available"}
            </h2>
            <p className={styles.locationCoords}>
              Latitude: {farm.location.latitude.toFixed(5)}, Longitude:{" "}
              {farm.location.longitude.toFixed(5)}
            </p>
            <h4 className={styles.sectionTitle}>
              Next Year Weather Averages Predictions
            </h4>
            {loadingPastStats ? (
              <p>Loading stats...</p>
            ) : pastStats ? (
          ...
          <section className={styles.section}>
            ...
            <div className={styles.cropInfo}>
              Total Acres: {totalAcres.toFixed(1)} acre
              {totalAcres !== 1 ? "s" : ""}
              <br />
            ...
            <ul className={styles.cropList}>
              {Object.entries(farm.plants)
                .sort(
                  ([, acresA], [, acresB]) =>
                    (Number(acresB) || 0) - (Number(acresA) || 0)
                ) // Sort descending by acres
                .map(([plant, acres]) => {
                  const numericAcres = Number(acres) || 0;
                  const percentage =
                    totalAcres > 0
                      ? Math.round((numericAcres / totalAcres) * 100)
                      : 0;
                  return (
                    <li key={plant} className={styles.cropListItem}>
                      {plant}: {percentage}% - {numericAcres.toFixed(1)} acre
                      {numericAcres !== 1 ? "s" : ""}
                    </li>
                  );
                })}
              ...
              {alerts && alerts.length > 0 ? (
                alerts.map((alert, index) => (
                  <li key={index} className={styles.alertListItem}>
                    <strong>{alert.event || alert.headline || "Alert"}</strong>:{" "}
                    {alert.desc || "No description available."}
                  </li>
                ))
              ) : (
                <li className={styles.noAlerts}>no weather alerts</li>
              )}
            </ul>
          )}
        </section>
              ...
              {forecastDays && forecastDays.length > 0 ? (
                forecastDays.map((day) => (
                  <div key={day.date} className={styles.forecastCard}>
                    ...
                    <p className={styles.forecastDetail}>
                      <strong>High:</strong> {day.day.maxtemp_c.toFixed(0)}°C /{" "}
                      {day.day.maxtemp_f.toFixed(0)}°F
                    </p>
                    <p className={styles.forecastDetail}>
                      <strong>Low:</strong> {day.day.mintemp_c.toFixed(0)}°C /{" "}
                      {day.day.mintemp_f.toFixed(0)}°F
                    </p>
                    <p className={styles.forecastDetail}>
                      <strong>Wind:</strong> {day.day.maxwind_kph.toFixed(0)}{" "}
                      kph
                    </p>
                    <p className={styles.forecastDetail}>
                    ...
                    <p className={styles.forecastDetail}>
                      <strong>Rain:</strong> {day.day.daily_chance_of_rain}%
                    </p>
